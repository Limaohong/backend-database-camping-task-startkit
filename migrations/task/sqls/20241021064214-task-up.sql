...
values
((select id from "USER" where name = '王小明'),(select id from "CREDIT_PACKAGE" where credit_amount = 14),14,(select price from "CREDIT_PACKAGE" where credit_amount = 14)),
((select id from "USER" where name = '王小明'),(select id from "CREDIT_PACKAGE" where credit_amount = 21),21,(select price from "CREDIT_PACKAGE" where credit_amount = 21)),
((select id from "USER" where name = '好野人'),(select id from "CREDIT_PACKAGE" where credit_amount = 14),14,(select price from "CREDIT_PACKAGE" where credit_amount = 14));
...
update "COURSE_BOOKING" set 
    status = '課程已取消',cancelled_at = '2024-11-24 17:00:00' 
where 
...
set 
	join_at = '2024-11-25 14:01:59',status = '上課中' 
where 
	user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') 
	and status = '即將授課' 
...
where
	cp.user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
	group by cp.user_id;
...
from
	(
		select 
			SUM(cp_sub.purchased_credits) as total_credit, 
			cp_sub.user_id 
		from "CREDIT_PURCHASE" cp_sub 
		group by cp_sub.user_id
	) as CREDIT_PURCHASE
inner join 
	(
		select 
			...
			cb_sub.user_id 
		from "COURSE_BOOKING" cb_sub 
		where 
			status != '課程已取消' 
		group by cb_sub.user_id
	) as COURSE_BOOKING 
...
from "SKILL" s
left join "COACH_LINK_SKILL" cls on cls.skill_id = s.id
group by s.id
...
select
	cp2.name as 組合包方案名稱,
	COUNT(cp.credit_package_id) as 銷售數量
from "CREDIT_PURCHASE" cp 
inner join "CREDIT_PACKAGE" cp2 on cp.credit_package_id = cp2.id
where
	cp.purchase_at between '2024-11-01 00:00:00' AND '2024-11-30 23:59:59'
group by cp2.id;
